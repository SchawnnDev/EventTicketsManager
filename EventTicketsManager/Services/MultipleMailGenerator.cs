...
public class MultipleMailGenerator
{
    ...
    public MultipleMailGenerator(SaveableEvent saveableEvent, List<MultipleMail> multipleMails, IConverter converter,
        ServerContext serverContext, string apiKey, string userId)
    {
    ...
    public async Task SendAllMailsAsync()
    {
        ...
        foreach (var multipleMail in _multipleMails)
        {
            ...
            var msg = MailHelper.CreateSingleEmail(from, to, subject, "",
                ConvertHtmlContent(htmlContent, multipleMail.Ticket));
            ...
            await using (var stream = new MemoryStream(pdfBytes))
            {
                await msg.AddAttachmentAsync(
                    $"Billet0{multipleMail.Ticket.Id}_{multipleMail.Ticket.FirstName}{multipleMail.Ticket.LastName}.pdf",
                    stream);
            }

            await _client.SendEmailAsync(msg);

            _serverContext.TicketUserMails.Add(
                new SaveableTicketUserMail(multipleMail.Ticket, _userId, DateTime.UtcNow));
    ...
    private string ConvertHtmlContent(string htmlContent, SaveableTicket ticket)
    {
        ...
        return htmlContent.Replace("{event.name}", _saveableEvent.Name)
            ...
            .Replace("{event.address}",
                string.IsNullOrEmpty(_saveableEvent.AddressNumber) || _saveableEvent.AddressNumber.Equals("0")
                    ? _saveableEvent.AddressName
                    : $"{_saveableEvent.AddressNumber} {_saveableEvent.AddressName}")
            .Replace("{event.postalCode}", _saveableEvent.PostalCode)
            .Replace("{event.start}",
                $"{_saveableEvent.Start.ToString("dd/MM/YYYY à HH:mm", specificCulture)}".Replace(":", "h")
                    .Replace("YYYY", Math.Abs(_saveableEvent.Start.Year - 2000).ToString()))
            .Replace("{event.end}",
                $"{_saveableEvent.End.ToString("dd/MM/YYYY à HH:mm", specificCulture)}".Replace(":", "h")
                    .Replace("YYYY", Math.Abs(_saveableEvent.End.Year - 2000).ToString()))
