    ...
    private string GetHtmlContent()
    {
        ...
        content = content.Replace("{event.name}", Ticket.Event.Name)
            ...
            .Replace("{event.address}",
                string.IsNullOrEmpty(Ticket.Event.AddressNumber) || Ticket.Event.AddressNumber.Equals("0")
                    ? Ticket.Event.AddressName
                    : $"{Ticket.Event.AddressNumber} {Ticket.Event.AddressName}")
            .Replace("{event.postalCode}", Ticket.Event.PostalCode)
            .Replace("{event.start}",
                $"{Ticket.Event.Start.ToString("dd/MM/YYYY à HH:mm", specificCulture)}".Replace(":", "h")
                    .Replace("YYYY", Math.Abs(Ticket.Event.Start.Year - 2000).ToString()))
            .Replace("{event.end}",
                $"{Ticket.Event.End.ToString("dd/MM/YYYY à HH:mm", specificCulture)}".Replace(":", "h")
                    .Replace("YYYY", Math.Abs(Ticket.Event.End.Year - 2000).ToString()))
